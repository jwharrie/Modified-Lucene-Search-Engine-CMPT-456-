...
public class HtmlIndexFiles {
  ...
  static void indexDoc(IndexWriter writer, Path file, long lastModified) throws IOException {
    try (InputStream stream = Files.newInputStream(file)) {
      ...
      // NEW: parsing for title and text content only
      BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8));
      DemoHTMLParser htmlParser = new DemoHTMLParser();
      DocData midDoc = htmlParser.parse(new DocData(), "content", new Date(), bufferedReader, new TrecContentSource());
      String fileContents = midDoc.getTitle();
      fileContents = fileContents + " " + midDoc.getBody();
